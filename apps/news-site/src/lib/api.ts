...
export async function getPostBySlug(slug: string, fields = []) {
  ...
  const {
    data,
    error,
  } = await databaseClient(DB_URL, DB_KEY).from('articles').select(fields.join(', ')).eq('slug', slug).single();
  if (!data?.length || error) return [];
...
export async function getAllPosts(fields = []) {
  ...
  const {
    data,
    error,
  } = await databaseClient(DB_URL, DB_KEY).from('articles').select(fields.join(', ')).order('created_at', {ascending: false});
  if (!data?.length || error) return [];
